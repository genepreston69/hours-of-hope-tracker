...
-- Enable RLS on routes_data only (location_hours_summary is a view)
ALTER TABLE public.routes_data ENABLE ROW LEVEL SECURITY;

-- Add RLS policies for routes_data
...
-- Get the default organization ID and update existing records
DO $$ 
DECLARE
    default_org_id uuid;
BEGIN
